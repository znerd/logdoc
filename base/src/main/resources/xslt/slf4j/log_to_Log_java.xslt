 ...
 */
		...
		<xsl:text>class Log extends org.znerd.logdoc.internal.slf4j.AbstractLog {
    ...
    * Logger for the entry with ID </xsl:text>
		...
		<xsl:text>.
    */
   private static org.slf4j.Logger LOGGER_</xsl:text>
		<xsl:value-of select="@id" />
		<xsl:text>;</xsl:text>
	</xsl:template>
   ...
   public static final void log_]]></xsl:text>
      ...
      if (LOGGER_</xsl:text>
		...
		<xsl:text> == null) {
         LOGGER_</xsl:text>
		<xsl:value-of select="@id" />
		<xsl:text> = org.slf4j.LoggerFactory.getLogger("</xsl:text>
      ...
      if (LOGGER_</xsl:text>
		...
		<xsl:choose>
			...
			<xsl:when test="@level = 'NOTICE'" >Info</xsl:when>
			<xsl:when test="@level = 'WARNING'">Warn</xsl:when>
			<xsl:when test="@level = 'ERROR'  ">Error</xsl:when>
			<xsl:when test="@level = 'FATAL'  ">Error</xsl:when>
			<xsl:otherwise>
				<xsl:message terminate="yes">Unregognized level <xsl:value-of select="@level" />.</xsl:message>
			</xsl:otherwise>
		</xsl:choose>
		<xsl:text>Enabled()) {
         String _translation = </xsl:text>
		<xsl:choose>
			<xsl:when test="@level = 'NOTICE'"><xsl:text> "NOTICE: " + </xsl:text></xsl:when>
			<xsl:when test="@level = 'FATAL'" ><xsl:text> "FATAL: " + </xsl:text></xsl:when>
		</xsl:choose>
